(ns auxiliary.operation
  "OperationActor -- one insurance-auxiliary operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Claims-LLM) is sealed into a single node (:advise); its proposal is
  ALWAYS routed through the Insurance Auxiliary Governor (:govern) and
  ...
  One graph run = one insurance-auxiliary operation (intake -> advise ->
  ...
  decision to a human operator (the licensed administrator/average
  adjuster). The approver resumes with `{:approval {:status
  :approved}}` (or :rejected). `:recommendation/finalize` ALWAYS reaches
  this node when the governor is clean -- see `auxiliary.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [auxiliary.claimsllm :as claimsllm]
            [auxiliary.governor :as governor]
...
(defn build
  ...
  opts:
    :advisor      -- an `auxiliary.claimsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (claimsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Claims-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (claimsllm/-advise advisor store request)]
            {:proposal p :audit [(claimsllm/trace request p)]})))

      ;; Insurance Auxiliary Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
